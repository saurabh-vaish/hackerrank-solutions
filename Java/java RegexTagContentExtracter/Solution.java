...
public class Solution{
	...
	/*here
           ...
           () = for one group
            .  = for all content
            + = all one or many
            ? = will accept at leat charecters i.e. <(.+?)> will search for only first tag   
            [^<>] = for except <> as in case of nested tag only content will come 
            </ =  for </ end tag
            1 = for first group i.e. starting tag 
            > = close with >
         ...
         */	
            ...
            while (m.find())
            {
                System.out.println(m.group(2)); // print 2nd group content i.e. ([^<>]+)
                match = true;
            }
            if(!match)
            {
